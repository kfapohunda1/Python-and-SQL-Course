...
tesla_data=Tesla_stock.history(period='max') #this gives us the max share price for each day
tesla_data.head()
...
html_data=requests.get(url).text #to request the html data from given URL
...
gme_revenue=gme_revenue[gme_revenue['Revenue']!=""] #this removes empty rows
gme_revenue
